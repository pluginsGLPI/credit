...
install_glpi() {
   ...
   cp -r ../credit plugins/$PLUGINNAME
}
...
# Plugin upgrade test
plugin_test_upgrade() {
   : mysql -h$DB_HOST -u$DB_USER -p$DB_PASSWD $OLD_DB_NAME < tests/plugin_credit_config_1.8.0.sql
   : mysql -h$DB_HOST -u$DB_USER -p$DB_PASSWD $OLD_DB_NAME < tests/plugin_credit_empty_1.8.0.sql
   : php ../../bin/console glpi:plugin:install credit --username=glpi --config-dir=../../$TEST_GLPI_CONFIG_DIR
}
